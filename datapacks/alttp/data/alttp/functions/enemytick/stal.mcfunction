...
#Models
execute if block ~ ~-0.01 ~ #gothrough run data merge entity @s {HandItems:[{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:274}},{}]}
execute if block ~ ~-0.01 ~ #gothrough run scoreboard players set @s enemydummy2 0
execute unless block ~ ~-0.01 ~ #gothrough run data merge entity @s {HandItems:[{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:273}},{}]}
execute unless block ~ ~-0.01 ~ #gothrough if score @s enemydummy2 matches 0 run particle block pink_candle[candles=2] ~ ~0.25 ~ 0.2 0.1 0.2 0.2 15 force
