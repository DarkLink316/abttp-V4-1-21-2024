#FIND LINKED ENTITY
tag @s add link_a
execute as @e[type=silverfish,tag=kodongosf] if score @s enemylink = @e[type=armor_stand,tag=kodongo,limit=1,sort=nearest] enemylink run tag @s add link_b
...
execute if score @s enemydummy matches 60 run function alttp:kodongoshoot
