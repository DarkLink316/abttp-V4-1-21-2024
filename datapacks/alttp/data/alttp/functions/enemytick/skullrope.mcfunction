#FIND LINKED ENTITY
tag @s add link_a
execute as @e[type=zombie,tag=skullropezo] if score @s enemylink = @e[type=armor_stand,tag=skullrope,limit=1,sort=nearest] enemylink run tag @s add link_b

#MOVEMENT
execute if score @s enemydummy matches 1 run tp @s ~0.1 ~ ~ -90 0
execute if score @s enemydummy matches 2 run tp @s ~-0.1 ~ ~ 90 0
execute if score @s enemydummy matches 3 run tp @s ~ ~ ~0.1 0 0
execute if score @s enemydummy matches 4 run tp @s ~ ~ ~-0.1 180 0
...
execute at @s unless block ~ ~ ~ #gothrough run tp @s ^ ^ ^-0.2
...
execute at @s if score @s enemydummy matches ..-1 if block ^ ^ ^0.5 #gothrough run tp @s ^ ^ ^0.1
