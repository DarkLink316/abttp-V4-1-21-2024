#FIND LINKED ENTITY
tag @s add link_a
execute as @e[type=zombie,tag=wizzrobezo] if score @s enemylink = @e[type=armor_stand,tag=wizzrobe,limit=1,sort=nearest] enemylink run tag @s add link_b
execute as @e[type=silverfish,tag=wizzrobesf] if score @s enemylink = @e[type=armor_stand,tag=wizzrobe,limit=1,sort=nearest] enemylink run tag @s add link_c
...
execute if score @s enemydummy matches 0 if entity @p[distance=..10] run function alttp:wwizzrobe_shoot

execute if score @s enemydummy matches 1.. unless entity @s[tag=icerodfrozen] run scoreboard players remove @s enemydummy 1

#TELEPORTY TELEPORTY?
execute unless entity @s[tag=icerodfrozen] run scoreboard players add @s enemydummy2 1
execute if score @s enemydummy2 matches 80 store result score result rand run random value 1..4
...
execute if score @s enemydummy9 matches 0 run data merge entity @s {HandItems:[{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:129}},{}]}
...
execute unless score @s enemydummy9 matches 0 if score @s enemydummy8 matches 1..5 run data merge entity @s {HandItems:[{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:130}},{}]}
execute unless score @s enemydummy9 matches 0 if score @s enemydummy8 matches 6..10 run data merge entity @s {HandItems:[{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:131}},{}]}

#SHOOTING ANIMATION
execute if score @s enemydummy matches ..1 if entity @p[distance=..10] run data merge entity @s {HandItems:[{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:134}},{}]}
execute if score @s enemydummy matches 2..5 if entity @p[distance=..10] run data merge entity @s {HandItems:[{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:133}},{}]}
execute if score @s enemydummy matches 6..9 if entity @p[distance=..10] run data merge entity @s {HandItems:[{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:132}},{}]}

#DAMAGE INDICATOR
execute if entity @e[type=armor_stand,tag=link_b,nbt={HurtTime:10s}] run particle block redstone_block ~ ~0.7 ~ 0 0 0 0.1 50 force
