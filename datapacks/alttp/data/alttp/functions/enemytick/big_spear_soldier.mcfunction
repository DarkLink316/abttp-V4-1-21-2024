#FIND LINKED ENTITY
tag @s add link_a
execute as @e[type=zombie,tag=big_spear_soldierzo] if score @s enemylink = @e[type=armor_stand,tag=big_spear_soldier,limit=1,sort=nearest] enemylink run tag @s add link_b

#THROW
scoreboard players add @s enemydummy2 1
execute if score @s enemydummy2 matches 40 if entity @p[distance=..10] run function alttp:big_spearthrow
