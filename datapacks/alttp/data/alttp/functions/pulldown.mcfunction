execute at @s if block ~ ~-1 ~ #gothrough run tp @s ~ ~-1 ~
execute at @s if block ~ ~-1 ~ #gothrough run tp @s ~ ~-1 ~
execute at @s if block ~ ~-0.5 ~ #gothrough run tp @s ~ ~-0.5 ~
execute at @s if block ~ ~-0.4 ~ #gothrough run tp @s ~ ~-0.4 ~
execute at @s if block ~ ~-0.3 ~ #gothrough run tp @s ~ ~-0.3 ~
execute at @s if block ~ ~-0.2 ~ #gothrough run tp @s ~ ~-0.2 ~
execute at @s if block ~ ~-0.1 ~ #gothrough run tp @s ~ ~-0.1 ~
execute at @s if block ~ ~-0.01 ~ #gothrough run tp @s ~ ~-0.01 ~
execute at @s if block ~ ~-0.01 ~ #gothrough run tp @s ~ ~-0.01 ~

execute at @s if block ~ ~-0.05 ~ #gothrough run function alttp:pulldown